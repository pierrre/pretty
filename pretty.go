...
func (vw *UintptrValueWriter) WriteValue(c *Config, w io.Writer, st *State, v reflect.Value) bool {
	...
	writeString(w, "0x")
	mustWrite(strconvio.WriteUint(w, v.Uint(), 16))
	return true
}
...
// NewFuncValueWriter creates a new [FuncValueWriter].
...
// NewPointerValueWriter creates a new [PointerValueWriter].
...
func (vw *UnsafePointerValueWriter) WriteValue(c *Config, w io.Writer, st *State, v reflect.Value) bool {
	...
	writeString(w, "0x")
	mustWrite(strconvio.WriteUint(w, uint64(uintptr(v.UnsafePointer())), 16))
	return true
}
...
type CommonValueWriter struct {
	...
	Reflect         *ReflectValueWriter
	Error           *ErrorValueWriter
	BytesHex        *BytesHexValueWriter
	BytesableHex    *BytesableHexValueWriter
...
func NewCommonValueWriter() *CommonValueWriter {
	...
	vw.PanicRecover = NewPanicRecoverValueWriter(vw.unwrapInterface)
	vw.UnwrapInterface = NewUnwrapInterfaceValueWriter(vw.recursion)
	vw.Recursion = NewRecursionValueWriter(vw.maxDepth)
	vw.MaxDepth = NewMaxDepthValueWriter(vw.typeAndValue)
	vw.TypeAndValue = NewTypeAndValueWriter(vw.writeType, vw.internal)
	vw.Type = NewTypeValueWriter()
	vw.Kind = NewKindValueWriter(vw.loopback)
	vw.Reflect = NewReflectValueValueWriter(vw.loopback)
	vw.Error = NewErrorValueWriter()
	vw.BytesHex = NewBytesHexValueWriter()
	vw.BytesableHex = NewBytesableHexValueWriter()
...
func (vw *CommonValueWriter) reflect(c *Config, w io.Writer, st *State, v reflect.Value) bool {
	if vw.Reflect == nil {
		return false
	}
	return vw.Reflect.WriteValue(c, w, st, v)
}
...
func (vw *CommonValueWriter) bytesHex(c *Config, w io.Writer, st *State, v reflect.Value) bool {
	if vw.BytesHex == nil {
		return false
	}
	return vw.BytesHex.WriteValue(c, w, st, v)
}

func (vw *CommonValueWriter) bytesableHex(c *Config, w io.Writer, st *State, v reflect.Value) bool {
	if vw.BytesableHex == nil {
		return false
	}
	return vw.BytesableHex.WriteValue(c, w, st, v)
}
...
//
// It should be created with [NewReflectValueValueWriter].
type ReflectValueWriter struct {
	ValueWriter
}

// NewReflectValueValueWriter creates a new [ReflectValueWriter].
func NewReflectValueValueWriter(vw ValueWriter) *ReflectValueWriter {
...
// BytesHexValueWriter is a [ValueWriter] that handles []byte and writes them with [hex.Dumper].
//
// It should be created with [NewBytesHexValueWriter].
type BytesHexValueWriter struct {
	// ShowLen shows the len.
	// Default: true.
	ShowLen bool
	// ShowCap shows the cap.
...
// NewBytesHexValueWriter creates a new [BytesHexValueWriter].
func NewBytesHexValueWriter() *BytesHexValueWriter {
	return &BytesHexValueWriter{
		ShowLen: true,
		ShowCap: true,
		MaxLen:  0,
	}
...
// WriteValue implements [ValueWriter].
func (vw *BytesHexValueWriter) WriteValue(c *Config, w io.Writer, st *State, v reflect.Value) bool {
	if v.Type() != bytesType {
		return false
	}
	if checkNil(w, v) {
	...
	b := v.Bytes()
	writeBytesHexCommon(c, w, st, b, vw.ShowLen, vw.ShowCap, vw.MaxLen)
	return true
}
...
// BytesableHexValueWriter is a [ValueWriter] that handles [Bytesable] and writes thems with [hex.Dumper].
//
// It should be created with [NewBytesableHexValueWriter].
type BytesableHexValueWriter struct {
	// ShowLen shows the len.
	// Default: true.
	ShowLen bool
	// ShowCap shows the cap.
...
// NewBytesableHexValueWriter creates a new [BytesableHexValueWriter].
func NewBytesableHexValueWriter() *BytesableHexValueWriter {
	return &BytesableHexValueWriter{
		ShowLen: true,
		ShowCap: true,
		MaxLen:  0,
	}
...
// WriteValue implements [ValueWriter].
func (vw *BytesableHexValueWriter) WriteValue(c *Config, w io.Writer, st *State, v reflect.Value) bool {
...
func writeBytesHexCommon(c *Config, w io.Writer, st *State, b []byte, showLen bool, showCap bool, maxLen int) {
	if showLen {
		if showCap {
...
func writeLen(w io.Writer, ln int) {
	writeString(w, "(len=")
	mustWrite(strconvio.WriteInt(w, int64(ln), 10))
	writeString(w, ")")
}
...
func writeLenCap(w io.Writer, ln int, cp int) {
	...
	mustWrite(strconvio.WriteInt(w, int64(ln), 10))
	writeString(w, " cap=")
	mustWrite(strconvio.WriteInt(w, int64(cp), 10))
	writeString(w, ")")
}
